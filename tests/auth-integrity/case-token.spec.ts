...
import { flipOneChar, getMe, loginStorageState, roleCookieValue } from '../../src/auth/sessionHelpers';
 ...
 * be rejected with 401 "Invalid Case Token", proving the token is not decorative.
 * (Residual, out of scope here: the token is also accepted as a ?t= query param.)
 */
test.describe('PC-03 — case token validation (positive control)', () => {
  test('[PC-03] tampering one character of the case token is rejected with 401 Invalid Case Token', async () => {
    ...
    expect(res.body, 'rejection should indicate token validation').toContain('Invalid Case Token');
  });
});
